    ...
    form.addEventListener(
      ...
      function (event) {
        if (!form.checkValidity()) {
          ...
          ajaxPostFile('/kepailitan/simpan', data, 'success_res', 'error_res');
        }
    ...
    form.addEventListener(
      ...
      function (event) {
        if (!form.checkValidity()) {
          ...
          ajaxPostFile(`/kepailitan/update/${id}`, data, 'success_res', 'error_res');
        }
...
function getData() {
  table = $('.datatables-kepailitan').DataTable({
    processing: true,
    serverSide: true,
    searching: true,
    responsive: false,
    ...
    columns: [
      ...
      {
        data: 'type_kepailitan',
        name: 'type_kepailitan',
        responsivePriority: -1
      },
      {
        data: 'aksi',
...
$(document).on('click', '#tambah', function () {
  ...
  var inputGroup = $(
    '<div class="d-flex gap-2 row-add form-row mb-3 align-items-center">' +
      ...
      '<div class="col-md-2"><input type="text" class="form-control" name="email[]" placeholder="Email"></div>' +
      '<div class="col-md-2"><input type="number" class="form-control" name="notelp[]" placeholder="No Telp"></div>' +
...
function success_res(data) {
  if (data.status == 200) {
    ...
    window.location.href = '/kepailitan';
