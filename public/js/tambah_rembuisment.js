...
$(document).on('click', '#tambah', function () {
  ...
  var inputGroup = $(
    ...
    <div class="col-lg-4 md-12">
        <div class="form-floating form-floating-outline mb-4">
            <input id="uraian${no}" class="form-control" name="uraian[${no}]" placeholder="Uraian..."></input>
    ...
    form.addEventListener(
      ...
      function (event) {
        if (!form.checkValidity()) {
          ...
          ajaxPostFile('/kegiatan/rembuisment/simpan', form, 'success_res', 'error_res');
        }
...
function getAgendaKegiantan(id) {
  ajaxGetWithoutLoading('/laporan/get-laporan-kegiatan/' + id, 'success_list_agenda', 'error_list_agenda');
}
...
function success_list_agenda(data) {
  if (data.status == 200) {
    ...
    data.data.forEach(element => {
      html += `<option value="${element['id']}" tempat="${element['tempat']}" tanggal="${element['tanggal']}">${element['judul']}</option>`;
    });
    console.log(html);
  ...
  if (data.status == 200) {
    ...
    swalWithBootstrapButtons
      .fire({
        ...
        confirmButtonText: 'Lihat Preview laporan',
        cancelButtonText: 'Tambah Kegiatan',
        reverseButtons: true
      })
      .then(result => {
        if (result.isConfirmed) {
          window.location.href = '/laporan/preview/' + $('#agenda').val();
        } else {
          $('#nama_kegiatan').val('');
          $('#tempat_kegiatan').val('');
