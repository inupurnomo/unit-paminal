...
@section('title', ' Edit Kegiatan - Forms')
...
{{-- @php
    dd($kegiatan_kuasa_hukum->pluck('id_user')->toArray());
@endphp --}}
    <h4 class="py-3 mb-4">
        <span class="text-muted fw-light">Kegiatan /</span> Edit Kegiatan
    </h4>
    <!-- Basic Layout -->
    <div class="row">
        <div class="col-xl">
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Form Edit Kegiatan</h5> <small class="text-body float-end"></small>
                </div>
                <div class="card-body">
                    <form class="forms-edit-validation" onsubmit="return false;" novalidate id="formeditkegiatan" enctype="multipart/form-data">
                        ...
                        <div class="form-floating form-floating-outline mb-4">
                            <select id="type_kegiatan" class="select2 form-select" name="type_kegiatan"
                                ...
                                <option value="Sidang" {{ $data->type_kegiatan == "Sidang" ? "Selected" : "" }}>Sidang</option>
                                <option value="Non Sidang" {{ $data->type_kegiatan == "Non Sidang" ? "Selected" : "" }}>Non Sidang</option>
                            </select>
                            <label for="type_kegiatan">Jenis Kegiatan</label>
                        </div>
                        ...
                        <input type="hidden" id="jumlahPihak" value="{{ count($kegiatan_pemberikuasa) }}">
                        <label>Pemberi Kuasa</label>
                        <div id="dynamicInputContainer" class="d-flex flex-column">
                                @if (count($kegiatan_pemberikuasa) > 0)
                                    @foreach ($kegiatan_pemberikuasa as $item)
                        ...
                        <div class="col-md-12 select2-primary mb-4">
                            <div class="form-floating form-floating-outline">
                                <select name="kuasa_hukum[]" class="kuasa_hukum select2 form-select" multiple>
                                    @foreach ($kuasa_hukum as $value)
                                        <option value="{{ $value->id }}"  {{ in_array($value->id, $kegiatan_kuasa_hukum->pluck('id_user')->toArray()) ? 'selected' : '' }}>{{ $value->name }}</option>
                                    @endforeach
                                </select>
